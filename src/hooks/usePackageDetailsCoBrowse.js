...
export const usePackageDetailsCoBrowse = (userType = 'agent', enabled = true) => {
  ...
  // Refs to prevent signal loops
  const isIncomingActionRef = useRef(false);
  ...
  }, [enabled, handleTabChange, handleImageSelect, handleDaySelect, handleFullscreenToggle, 
      handleSlideshowToggle, handleImageNavigate, handleZoomChange, handleScrollSync, 
      handleComparisonAction, handlePaymentAction, handleModalOpen, handleModalClose]);

  // Action senders
  const sendTabChange = useCallback((tabIndex) => {
    if (!enabled || isIncomingActionRef.current) return;
  ...
  return {
    ...
    isIncomingAction: isIncomingActionRef.current,
